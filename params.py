with st.container(border=True):
    ...
    if page == "个股分析":
        ...
        with command_cols[3]:
            run = st.button("来财来财", type="primary", use_container_width=True, key="run_stock_analysis")
        ...
        provider_label = st.session_state.get("ai_market_provider_label", "Gemini")
        assistant_status = AIMarketAssistant(config, provider="deepseek" if provider_label == "DeepSeek" else "gemini")
        ai_cols = st.columns([0.78, 0.22], vertical_alignment="center")
        with ai_cols[0]:
            st.caption(f"当前 API：{assistant_status.provider_label()} · {'已连接' if assistant_status.ready() else '未配置'}")
        with ai_cols[1]:
