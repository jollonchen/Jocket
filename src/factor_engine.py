...
class FactorEngine:
    ...
    def _short_breakdown(self, r, p, hist, ev) -> list[dict]:
        ...
        if ev.get('turnover_rate') is None:
            negative.append('当前数据源暂不支持或未返回换手率，流动性判断仍需补充验证')
        liq_item = self._item(
            '流动性', liq, 15,
    ...
    def _long_breakdown(self, r, hist, ev) -> list[dict]:
        quality = self._item(
            '质量占位', 10, 15,
            '当前数据源暂未接入 ROE、毛利率、经营现金流等财务质量指标，本项为占位分。',
            {'roe': '暂未接入', '毛利率': '暂未接入', '经营现金流': '暂未接入'},
            ['使用中性占位分，避免错误放大未知基本面优势'],
            ['未接入正式财务质量指标，不作为基本面判断依据'],
            '中长线质量判断数据受限，需要补充财报和现金流数据。'
        )
        growth = self._item(
            '成长占位', 8, 15,
            '当前版本暂未接入营收同比、净利润同比、扣非净利润等成长指标，本项为占位分。',
            {'营收同比': '暂未接入', '净利润同比': '暂未接入', '扣非净利润': '暂未接入'},
            ['使用中性偏保守占位分'],
            ['缺少成长数据，不能据此判断业绩趋势'],
        ...
        valuation = self._item(
            '估值占位', 5, 10,
            '当前版本暂未接入 PE/PB/PS 等估值指标，本项暂不作为正式估值判断依据。',
            {'PE': '暂未接入', 'PB': '暂未接入', 'PS': '暂未接入'},
            ['使用中性估值占位分'],
            ['缺少估值分位，不能判断便宜或昂贵'],
    ...
    def _score_explanations(self, short_breakdown, long_breakdown, ev) -> list[str]:
        ...
        return [
            ...
            '中长线评分会在基本面模块可用时纳入财务质量、成长、估值与股息；若基本面数据不可用，则回退为趋势和保守占位项。',
    ...
    def _limitations(self, ev) -> list[str]:
        notes = [
            '若基本面模块未能返回财务/估值数据，质量/成长/估值将使用保守占位；UI 可用时会自动纳入财务估值',
    ...
    def _formula(self) -> dict:
        return {
            ...
            'long': {
                '质量占位': ['暂未接入ROE、毛利率、经营现金流，使用占位分10/15'],
                '成长占位': ['暂未接入营收/利润增速，使用占位分8/15'],
                '估值占位': ['暂未接入PE/PB/PS，使用占位分5/10'],
