...
from src.hedge_src.tools.api import get_prices, prices_to_df
...
def safe_int_percent(value, default=50):
    """
    Safely convert a percentage float to int, handling NaN cases
    """
    try:
        if pd.isna(value) or np.isnan(value):
            return default
...
def technical_analyst_agent(state: AgentState, agent_id: str = "technical_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        technical_analysis[ticker] = {
            ...
            "confidence": safe_int_percent(combined_signal["confidence"] * 100),
            "reasoning": {
                "trend_following": {
                    "signal": trend_signals["signal"],
                    "confidence": safe_int_percent(trend_signals["confidence"] * 100),
                    "metrics": normalize_pandas(trend_signals["metrics"]),
                },
                "mean_reversion": {
                    "signal": mean_reversion_signals["signal"],
                    "confidence": safe_int_percent(mean_reversion_signals["confidence"] * 100),
                    "metrics": normalize_pandas(mean_reversion_signals["metrics"]),
                },
                "momentum": {
                    "signal": momentum_signals["signal"],
                    "confidence": safe_int_percent(momentum_signals["confidence"] * 100),
                    "metrics": normalize_pandas(momentum_signals["metrics"]),
                },
                "volatility": {
                    "signal": volatility_signals["signal"],
                    "confidence": safe_int_percent(volatility_signals["confidence"] * 100),
                    "metrics": normalize_pandas(volatility_signals["metrics"]),
                },
                "statistical_arbitrage": {
                    "signal": stat_arb_signals["signal"],
                    "confidence": safe_int_percent(stat_arb_signals["confidence"] * 100),
