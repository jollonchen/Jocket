...
class BacktestEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # Determine benchmark ticker dynamically based on the first ticker
        self._benchmark_ticker = "SPY"
        if tickers:
            from src.utils import normalize_a_share_code
            first_ticker = str(tickers[0]).strip().upper()
            normalized = normalize_a_share_code(first_ticker)
            if normalized.endswith((".SS", ".SZ", ".HK")):
                self._benchmark_ticker = "000300.SS"
