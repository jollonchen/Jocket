...
def _run(ticker: str, trade_date: str, config: dict, tracker: ProgressTracker) -> None:
    ...
    # Resolve company name robustly
    from web.history import parse_company_name_from_json
    company_name = parse_company_name_from_json(None, ticker, last_chunk)
    if not company_name:
        company_name = getattr(tracker, "company_name", "")
    if not company_name:
        try:
            from src.tradingagents_ui import _resolve_stock_name
            company_name = _resolve_stock_name(ticker)
        except Exception:
            pass

    graph.ticker = ticker
    graph._log_state(trade_date, last_chunk, company_name=company_name)
